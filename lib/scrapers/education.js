...
module.exports = async () => {
  ...
  try {
    ...
    const events = $('#news dd').map((i, el) => {
      ...
      try {
        const data = {
          about: raw.match(/【(\S*)】/u)[1],
          department: '教育学部',
          eventDate: parseDate(raw, 'M月D日', raw.match(/日\s*\((\S)\)/u)[1]),
          link: baseURL + ($item.find('a').attr('href') || resourcePath),
          period: raw.match(/\)\s*(\S*)限/u)[1].replace('時', ''),
          pubDate: parseDate($item.find('.date').text(), 'YYYY/MM/DD'),
          raw,
          subject: raw.match(/「(.*)」/u)[1],
          teacher: raw.match(/」\s*\((.*)教員\)/u)[1]
        };
        if (/地区開講/u.test(raw)) {
          [, data.campus] = raw.match(/限\s*(\S*)開講/u);
        }
        if (/教室/u.test(raw)) {
          [, data.room] = raw.match(/教室:(.*)/u);
        }
        return data;
      } catch (err) {
