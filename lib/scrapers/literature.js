...
module.exports = async () => {
  const resourceURL = 'https://www2.lit.kyushu-u.ac.jp/~syllabus/cgi-bin/class-schedule-1.cgi';
  try {
    ...
    const events = $('table tr:first-child table tr:not(:first-child)').map((i, el) => {
      ...
      try {
        return {
          ...
          period: periodAndSubject.match(/.*曜(?<period>\d*)限/u).groups.period,
          pubDate: parseDate(normalizeText($item.find(':nth-child(6)').text()), 'YYYY年M月D日h時m分'),
          raw,
          subject: periodAndSubject.match(/限\s*(?<subject>.*)$/u).groups.subject,
