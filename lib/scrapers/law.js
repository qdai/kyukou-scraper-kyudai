...
module.exports = () => {
  ...
  return fetch(resourceURL).then($ => $('.article-main [style="height: 600px; overflow: auto;"] table tr:not(:first-child)').map((i, el) => {
    ...
    try {
      ...
      data.eventDate = parseDate(normalizeText($item.find(':nth-child(2)').text()).replace(/\(.*/, ''), 'YYYY年M月D日');
      data.pubDate = parseDate(normalizeText($item.find(':nth-child(5)').text()), 'YYYY年M月D日h時m分');
