...
module.exports = () => {
  ...
  return fetch(resourceURL)
    .then($ => $('.article-main [style="height: 600px; overflow: auto;"] table tr:not(:first-child)').map((i, el) => {
      const $item = $(el);
      const raw = $item.text();
      try {
        const data = {
          about: normalizeText($item.find(':nth-child(6)').text()),
          department: '法学部',
          eventDate: parseDate(normalizeText($item.find(':nth-child(2)').text()).replace(/\(.*/, ''), 'YYYY年M月D日'),
          link: resourceURL,
          note: normalizeText($item.find(':nth-child(7)').text()),
          period: normalizeText($item.find(':nth-child(2)').text()).match(/曜・(\d+)限/)[1],
          pubDate: parseDate(normalizeText($item.find(':nth-child(5)').text()), 'YYYY年M月D日h時m分'),
          raw,
          subject: normalizeText($item.find(':nth-child(3)').text()).replace(/\(補講\)$/, ''),
