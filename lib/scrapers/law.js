    ...
    const events = $('#contents table.cancel tr:not(:first-child)').map((i, el) => {
      ...
      try {
        const data = {
          ...
          period: normalizeText($item.find('.canceldate').text()).match(/(?<period>\d+)限/u).groups.period,
          pubDate: parseDate(normalizeText($item.find('.writedate').text()), 'M月D日'),
          raw,
          subject: normalizeText($item.find('.subject').text()).replace(/\(補講\)$/u, ''),
          teacher: normalizeText($item.find('.author').text())
        };
        if (data.about === '公務' || data.about === 'その他') {
          if (/\(補講\)$/u.test($item.find('.subject').text())) {
            data.about = '補講';
          }
        }
        return data;
